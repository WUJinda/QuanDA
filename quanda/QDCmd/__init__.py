...
# Tushare 基本面数据
# from quanda.QDFactor.localize import (QA_ts_update_daily_basic,
#                                          QA_ts_update_inc,
#                                          QA_ts_update_industry,
#                                          QA_ts_update_namechange,
#                                          QA_ts_update_stock_basic)
# 东方财富爬虫
from quanda.QDSU.main import (QA_SU_crawl_eastmoney, QA_SU_save_bond_day,
...
class CLI(cmd.Cmd):
    ...
    def do_examples(self, arg):
        QA_util_log_info("quanda example")
        now_path = os.getcwd()
    ...
    def do_download_updatex(self, arg):
        """Download update_x.py script from scripts/update/"""
        now_path = os.getcwd()
        import shutil
        src_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), "scripts", "update", "update_x.py")
        dst_file = os.path.join(now_path, "update_x.py")
        if os.path.exists(src_file):
            shutil.copy(src_file, dst_file)
            print(f"Successfully copied update_x.py to {dst_file}")
        else:
            print(f"Source file not found: {src_file}")
        self.lastcmd = ""

    def do_download_updateall(self, arg):
        """Download update_all.py script from scripts/update/"""
        now_path = os.getcwd()
        import shutil
        src_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), "scripts", "update", "update_all.py")
        dst_file = os.path.join(now_path, "update_all.py")
        if os.path.exists(src_file):
            shutil.copy(src_file, dst_file)
            print(f"Successfully copied update_all.py to {dst_file}")
        else:
            print(f"Source file not found: {src_file}")
        self.lastcmd = ""
    ...
    def do_help(self, arg):
        ...
        QA_util_log_info("MORE EXAMPLE on https://github.com/quanda/QADemo")
